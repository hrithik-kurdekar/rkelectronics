...
async function fetchQuotaUsage() {
  if (isDemoMode()) {
    return {
      ...
      items: QUOTA_METRICS.map(({ id, metric, label }) => ({
        id,
        label,
        usedBytes: 0,
        limitBytes: FREE_TIER_LIMITS[metric],
        percent: 0,
        unlimited: false,
      })),
      message: null,
...
export async function fetchDashboardMetrics() {
  ...
  const [supabaseHealth, keepAlive, traffic, quota] = await Promise.all([
    isDemoMode()
      ? Promise.resolve({ status: 'ok', latencyMs: 42, message: 'Connected' })
      : supabasePing(),
    fetchLastKeepAlive(),
    fetchTrafficMetrics(),
    fetchQuotaUsage(),
  ...
  return {
    ...
    keepAlive: {
      ...keepAlive,
      cronConfigured: demoMode ? true : cronConfigured,
    },
    traffic,
    quota,
    warnings,
