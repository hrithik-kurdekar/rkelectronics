...
export const DEMO_WRITE_MESSAGE = 'Changes could not be saved. Please try again later.';
...
export async function fetchConnectionById(id) {
  if (!id) return { data: null, error: null };

  if (isDemoMode()) {
    const row = localConnections({ activeOnly: false }).find((c) => c.id === id) || null;
