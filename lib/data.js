  ...
  try {
    ...
    if (level === 'home') {
      ...
      sections = slice.map((root) => {
        const rootSubs = subsByRoot[root.id] || [];
        const brandsForSubs = {};
        for (const sub of rootSubs) {
          // Only brands that actually have products
          brandsForSubs[sub.id] = (brandsGrouped[sub.id] || []).filter((brand) =>
            (productsByBrandId.get(brand.id) || []).length > 0
          );
        }
        return {
          category: root,
